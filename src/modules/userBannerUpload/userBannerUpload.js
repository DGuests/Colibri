...
class UserBannerUpload extends BaseHandlerLambda {
  async main (event) {
    try {
      ...
      await s3
        .putObject({
          ...
          Bucket: process.env.BANNER_UPLOAD_BUCKET,
          ACL: 'public-read'
        })
        .promise()

      const url = `https://${process.env.BANNER_UPLOAD_BUCKET}.s3.amazonaws.com/${key}`
      ...
      await dbConnection.model('User').update(
        {
          profile_url: url
        },
        {
      ...
      console.log('🚀 ~ file: userBannerUpload.js ~ line 35 ~ UserBannerUpload ~ main ~ error', error)
      return this.handleError(error)
    }
  }
}

const userBannerUpload = new UserBannerUpload()

module.exports = userBannerUpload.main.bind(userBannerUpload)
